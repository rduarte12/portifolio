    ...
    <main className="container" style={{ marginTop: '40px', marginBottom: '60px' }}>
      ...
      <section style={{ marginBottom: '32px' }}>
        ...
        <p className="text-muted" style={{ marginTop: '8px' }}>
          A collection of our recent research works.
        </p>
      </section>
      ...
      <section className="flex-col gap-sm">
        {(() => {
          ...
          return (
            <>
              {filteredCount === 0 ? (
                <p className="text-muted" style={{ fontSize: '0.9rem' }}>No research works found for the selected tag.</p>
              ) : (
                postsFiltered.slice(visibleCount).map((post) => (
                  <FeedCard
                    key={post.id}
                    id={post.id}
                    title={post.title}
